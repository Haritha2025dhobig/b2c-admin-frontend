...
//     setSelectedRow(row);
//     setFormData({
//       // read from *_pk (server read-only) / fallback to nested id if present
...
//       alert(err?.response?.data ? JSON.stringify(err.response.data, null, 2) : "Network/unknown error");
//     }
//   };
...
function ServiceDeliveryCodesPageInner() {
  ...
  useEffect(() => {
    if (token && !didFetch.current) {
      didFetch.current = true;
      // fetch both so table can show names immediately
      fetchReferenceData();
      fetchCodes();
    }
  ...
  useEffect(() => {
    if (token && laundryId) {
      axios
        .get(`${BASE_URL}laundary/${laundryId}/`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <CommonTable
        ...
        columns={[
          { key: "id", header: "ID" },
          // just the name from nested object
          {
            key: "service",
            header: "SERVICE",
            value: (row) => (row.service?.service_name ?? ""),
          },
          {
            key: "service_period",
            header: "SERVICE PERIOD",
            value: (row) => (row.service_period?.service_period_name ?? ""),
          },
          {
            key: "price_type",
            header: "PRICE TYPE",
            value: (row) => priceTypeName(row.price_type),
          },
